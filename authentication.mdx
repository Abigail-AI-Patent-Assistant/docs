...
1. **Create** a key at the [Developer Portal](https://abigail.app/developers)
2. The raw key is shown **once** -- copy it immediately
3. Only the SHA-256 hash is stored server-side
4. **Revoke** a key anytime from the Developer Portal
...
```json
{
  ...
  "agent_suggestion": "This endpoint requires an Abigail API key. The user must create one at https://abigail.app/developers and provide it to you."
}
```
...
```json
{
  ...
  "agent_suggestion": "The API key is invalid. Ask the user to generate a new key at https://abigail.app/developers"
}
```
...
```json
{
  ...
  "agent_suggestion": "The API key is revoked. Ask the user to generate a new key at https://abigail.app/developers"
}
```
